...
## License compliance (SSPL)

This project is SSPL-licensed (`LICENSE`). Read `LICENSING.md` at the start
of every session in this repo, before any other work — it has the full
procedure: the mandatory disclosure, the branding-drift and bot-identity
triggers, and what to do (and not do) once one fires. Don't paraphrase it
from memory of a prior session; it's a compliance procedure, re-read it.
