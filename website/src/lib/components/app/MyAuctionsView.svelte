<script lang="ts">
	...
	const STATUS_CLASS: Record<string, string> = {
		active: 'bg-black/5 text-black/70 dark:bg-white/10 dark:text-white/70',
	...
	const auctions = createPaginatedList<AuctionRow, { rows: AuctionRow[]; total: number }>((offset) =>
		telegramTrpc.telegram.auctions.mine.query({ limit: PAGE_SIZE, offset }),
	);
	auctions.reset();
</script>

<Page class="pb-safe-24">
	<Navbar title="Meus leilões">
		{#snippet left()}
	...
	{:else if auctions.items.length === 0}
		<div class="p-8 text-center text-black/55 dark:text-white/55">Você ainda não criou nenhum leilão.</div>
	{:else}
		<List strong outline>
				...
				<li class="hairline-b last:hairline-b-none relative overflow-hidden">
					...
					>
						...
						<div class="ml-1 min-w-0 flex-1">
							...
							<span class={`mt-1 inline-block rounded-full px-2 py-0.5 text-xs font-semibold ${STATUS_CLASS[row.auction.status] ?? STATUS_CLASS.active}`}>
