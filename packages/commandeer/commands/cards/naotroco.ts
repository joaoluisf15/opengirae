...
export default class NaoTrocoCommand extends Command {
  static override info = {
    name: 'naotroco',
    description: 'Marca uma ou várias cartas como não trocáveis',
    usage: '/naotroco <id ou nome do card> [id2 id3 ...]',
  ...
  static override async execute(ctx: IncomingCommand, args: { cardsRaw: string }) {
    ...
    if (tokens.length === 1 || !tokens.every(t => /^\d+$/.test(t))) {
      ...
      if (!(await CardsDB.hasUserCard(user.id, card.id))) {
        await reply(ctx, '😂 Não troca o que? Você não tem esse card.')
        return
      }
      await CardsDB.setCardTradable(user.id, card.id, false)
      await reply(ctx, `🔒 **${escapeMarkdown(card.name)}** agora está marcado como não trocável.`)
      return
    }
    ...
    if (validIds.length === 0) {
      await reply(ctx, '😂 Não troca o que? Você não tem nenhum desses cards.')
      return
    }

    const cards = await CardsDB.getCardsByIds(validIds)
    const cardsById = new Map(cards.map(c => [c.id, c]))
    for (const id of validIds) await CardsDB.setCardTradable(user.id, id, false)
