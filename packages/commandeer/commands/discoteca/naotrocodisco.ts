...
export default class NaoTrocoDiscoCommand extends Command {
  static override info = {
    name: 'naotrocodisco',
    description: 'Marca um ou vários álbuns/singles como não trocáveis',
    usage: '/naotrocodisco <id ou nome> [id2 id3 ...]',
  }
  ...
  static override async execute(ctx: IncomingCommand, args: { entriesRaw: string }) {
    ...
    if (tokens.length === 1 || !tokens.every(t => /^\d+$/.test(t))) {
      ...
      if (!owned || owned.count < 1) {
        await reply(ctx, '😂 Não troca o que? Você não tem esse item.')
        return
      }
      await DiscotecaDB.setEntryTradable(user.id, entry.id, false)
      await reply(ctx, `🔒 **${escapeMarkdown(entry.name)}** agora está marcado como não trocável.`)
      return
    }
    ...
    if (validIds.length === 0) {
      await reply(ctx, '😂 Não troca o que? Você não tem nenhum desses itens.')
      return
    }

    const entries = await DiscotecaDB.getEntriesByIds(validIds)
    const entriesById = new Map(entries.map(e => [e.id, e]))
    for (const id of validIds) await DiscotecaDB.setEntryTradable(user.id, id, false)
    ...
    await reply(ctx, `🔒 **Marcados como não trocáveis:**\n${list}${skippedNote}`)
  }
}
