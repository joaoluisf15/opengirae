...
import { modifyTradeOffer, getActiveTradeSide } from './trade.cards'
...
export default class RemoveCommand extends Command {
  static override info = {
    name: 'remove',
    description: 'Remove rapidamente uma ou várias cartas da troca em andamento',
    usage: '/remove <nome ou ID do personagem> [ID2 ID3 ...]',
    aliases: ['rem'],
  }
  ...
  static override async execute(ctx: IncomingCommand) {
    ...
    if (tokens.length === 1 || !tokens.every(t => /^\d+$/.test(t))) {
      ...
      const message = await modifyTradeOffer(ctx.message.author.id, ctx.message.platform as 'telegram' | 'discord', card.id, 'remove')
      await reply(ctx, { content: message, photoUrl: card.imageUrl ?? undefined })
      return
    }

    if (tokens.length > MAX_CARDS) {
      await reply(ctx, `Você só pode remover até ${MAX_CARDS} cartas de uma vez.`)
      return
    }
    ...
    for (const id of ids) {
      ...
      if (message === 'Carta removida.') removed.push(`${card.rarityEmoji} \`${card.id}\`. **${escapeMarkdown(card.name)}**`)
