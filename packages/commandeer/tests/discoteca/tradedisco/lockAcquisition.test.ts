...
import { tryAcquireLock, lockKey } from "../../../services/discoteca/tradeLock";

// exercises the real tradeLock.ts (SET NX, not GET-then-SET) tradedisco.ts uses to guard "already in a trade"
describe("tradedisco lock acquisition race", () => {
  const telegramId = `test-tradedisco-lock-${Date.now()}`;
