...
- **`uploadFromUrl`'s content-type guess is unreliable for Telegram video URLs, and a wrong guess fails silently** — `guessImageType()` (`packages/common/utilities/storage.ts`) falls back to `application/octet-stream`/`.bin` when neither the fetch response's `content-type` header nor the source URL's extension look like a known type. For every Telegram video, that fallback fired (found in prod: 12/12 `/upload` video submissions since the feature launched got stored as `.bin`). Telegram's `sendVideo` then refuses to play a URL served as `application/octet-stream`, but the failure never surfaces: `reply()` → `settleReply()` (`packages/common/dbos/messaging.ts`) catches send errors and just returns `undefined`, logging once to stdout with no persistent trail — so the caller's `if (reviewMessageId) ...` branch silently no-ops and the row sits with `reviewChatId`/`reviewMessageId` both `NULL` forever, no error anywhere durable. Fix: **when the caller already knows the media type, pass it through instead of re-guessing** — `uploadFromUrl` now takes an optional `isVideo` param (used by `packages/commandeer/worker.ts`'s upload worker) that skips `guessImageType` entirely. If you add another caller that already knows its media is a video (or add other known-type hints), thread it through the same way rather than relying on the guess.
